...
QGVScene::QGVScene(const QString &name, QObject *parent, graphDirectionType type) : QGraphicsScene(parent)
{
  _context = new QGVGvcPrivate(gvContext());

  // depending on the type of graph wanted, initialize
  if (type == graphDirected)
    _graph = new QGVGraphPrivate(agopen(name.toLocal8Bit().data(), Agdirected, NULL));
  if (type == graphStrictDirected)
    _graph = new QGVGraphPrivate(agopen(name.toLocal8Bit().data(), Agstrictdirected, NULL));
  if (type == graphUndirected)
    _graph = new QGVGraphPrivate(agopen(name.toLocal8Bit().data(), Agundirected, NULL));
  if (type == graphStrictUndirected)
    _graph = new QGVGraphPrivate(agopen(name.toLocal8Bit().data(), Agstrictundirected, NULL));
...
void QGVScene::setRootNode(QGVNode *node)
{
  ...
  QString str_containing_text_root("root");
  agset(_graph->graph(), str_containing_text_root.toLocal8Bit().data(), node->label().toLocal8Bit().data());
}
